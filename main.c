...
//取下一个参数
void get_parameter(char* str , int pos , char* paras)
{
	...
	if(pos == 0)
	{
		para = strtok(tmp,nbsp);
		paras = para;
		return;
	}
	else 
	{
		strtok(tmp,nbsp);//fetch the first parameters
		++cnt;

		while( (para = strtok(NULL,nbsp)) )
		{
			if(cnt == pos)
			{
				paras = para;
...
void TestB()
{
	...
	while (1) {
		...
		if (strcmp(cmd, "hello") == 0) // hello cmd
		｛
			printf("hello world!\n");

		｝
		else if (strcmp(cmd , "touch")) //创建新文件
		{
			int fd;
